...
import {
  ...
  computeVam,
  computeCombinedDistanceData,
  computeCombinedTimeData,
  extractRoutePoints,
  extractLaps,
  formatSplitPace,
  formatTime,
} from "@/lib/trackpoint-charts";
import { HrZoneBar, VamCard, CombinedMetricsChart } from "@/components/training/training-charts";
...
const RouteMap = dynamic(() => import("@/components/training/route-map").then((m) => m.RouteMap), {
  ssr: false,
});
...
function SourceBadge({ source }: { source: string }) {
  ...
  return <Badge variant={variant}>{sourceKey ? labelsT("sources." + sourceKey) : source}</Badge>;
}
...
export function ActivityCard({
  ...
  const labelsT = useTranslations("labels");
  const pace =
    log.distanceMeters && log.distanceMeters > 0 ? log.distanceMeters / log.durationSeconds : 0;
  ...
  async function handlePromote() {
    ...
    if (
      !log ||
      !window.confirm(
        t("card.setRaceGoal", { name: log.name, distance: distanceText, elevation: elevText })
      )
    )
      return;
  ...
  const hrZones =
    hasTrackpoints && maxHr ? computeHrZoneBreakdown(trackPoints!, maxHr, restingHr) : null;
    ...
    <Card>
        ...
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          {log.subType && (
            <Badge variant="secondary">
              {labelsT.has("subTypes." + log.subType)
                ? labelsT("subTypes." + log.subType)
                : log.subType.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase())}
            </Badge>
          )}
          <SourceBadge source={log.source} />
          {log.tss && (
            <Badge variant="outline">
              {t("card.tss")} {Math.round(log.tss)}
            </Badge>
          )}
          {log.remarks && (
            <Badge variant="secondary" className="gap-1">
              <MessageSquare className="h-3 w-3" /> {t("card.remarks")}
            </Badge>
          )}
          <button
            onClick={(e) => {
        ...
        {promoteResult?.success && (
          <div className="flex items-center gap-2 p-2 mb-2 rounded-md bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 text-xs text-green-800 dark:text-green-200">
            ...
            <span>
              ...
              <a href="/settings/goals" className="underline font-medium">
                {t("card.viewInSettings")}
              </a>
            </span>
          </div>
        )}
        ...
        {duplicateGroup && duplicateGroup.status === "pending" && (
          <div className="flex items-center gap-2 p-2 mb-2 rounded-md bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 text-xs text-amber-800 dark:text-amber-200">
            ...
            <span>
              ...
              <a href="/duplicates" className="underline font-medium">
                {t("card.reviewDuplicates")}
              </a>
            </span>
          </div>
        )}
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 mb-6">
          ...
          {log.distanceMeters && (
            <Stat
              ...
              value={formatDistance(log.distanceMeters)}
            />
          )}
          {log.elevationGainMeters && (
            <Stat
              ...
              value={formatDistance(log.elevationGainMeters)}
            />
          )}
          ...
          {log.averageHr && (
            <Stat
              icon={Heart}
              label={t("card.heartRate")}
              value={`${Math.round(log.averageHr)}${log.maxHr ? `/${Math.round(log.maxHr)}` : ""} bpm`}
            />
          )}
          {log.averagePower && (
            <Stat
              icon={Zap}
              label={t("card.power")}
              value={`${Math.round(log.averagePower)}${log.normalizedPower ? ` NP ${Math.round(log.normalizedPower)}` : ""}W`}
            />
          )}
          {log.calories && (
            <Stat
              icon={Flame}
              label={t("card.calories")}
              value={`${Math.round(log.calories)} kcal`}
            />
          )}
          {vam && (
            <Stat
              icon={TrendingUp}
              label={t("card.vam")}
              value={`${vam.vamTotal.toLocaleString()} m/h`}
            />
          )}
          {log.tss && (
            <Stat icon={BarChart3} label={t("card.tss")} value={String(Math.round(log.tss))} />
          )}
        ...
        {hasTrackpoints && combinedDistData.length >= 3 && (
          <div className="space-y-4 mb-4">
            ...
            <CombinedMetricsChart
              distanceData={combinedDistData}
              timeData={combinedTimeData}
        ...
        {vam && (
          <div className="mb-4">
            <VamCard
              totalGain={vam.totalGain}
              vamTotal={vam.vamTotal}
              peakVam30min={vam.peakVam30min}
            />
          </div>
        )}
        ...
        {similarRoutes.length > 0 && (
          <div className="mb-4">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-2">
              ...
              {t("card.sameRoute", { count: similarRoutes.length })}
            </h3>
                ...
                <tbody className="divide-y">
                  ...
                  <tr className="bg-primary/5 font-medium tabular-nums">
                    ...
                    <td className="px-3 py-1.5">
                      {log.averageHr ? `${Math.round(log.averageHr)} bpm` : "—"}
                    </td>
                  ...
                  {similarRoutes.map((m) => {
                    const mPace =
                      m.distanceMeters && m.distanceMeters > 0
                        ? m.distanceMeters / m.durationSeconds
                        : 0;
                    return (
                      <tr
                        key={m.id}
                        className="hover:bg-muted/30 tabular-nums cursor-pointer"
                      ...
                      >
                        ...
                        <td className="px-3 py-1.5">
                          ...
                          <span
                            className={`ml-1 text-[10px] ${deltaStr(log.durationSeconds, m.durationSeconds, "", true)}`}
                          >
                        ...
                        <td className="px-3 py-1.5">{m.tss ? Math.round(m.tss) : "—"}</td>
                        <td className="px-3 py-1.5">
                          <Badge
                            variant={
                              m.similarity >= 85
          ...
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              <span className="ml-auto flex items-center gap-2">
                ...
                {saved && (
                  <span className="text-xs text-green-600 font-normal">{t("card.saved")}</span>
                )}
                {remarksDirty && (
