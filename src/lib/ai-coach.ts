 ...
 *
 * The pure helpers, Zod schemas, and shared types live in
 * `ai-coach-utils.ts`; per-activity analysis lives in `ai-coach-activity.ts`.
 * Both are re-exported here so the public surface is unchanged.
 */
import { z } from "zod";
...
import {
  // Schemas (re-exported public surface)
  SuggestionSchema,
  AnalyzeResponseSchema,
  PlanProposalSchema,
  StartInterviewResponseSchema,
  // Pure helpers (re-exported public surface)
  buildContextSummary,
  sanitizeJsonText,
  derivePhaseStructure,
  getNextMondayStr,
  generatePhaseGoal,
  // Types (re-exported public surface)
...
// Re-export everything extracted into ai-coach-utils so existing imports of
// "./ai-coach" keep working without modification.
export {
  SuggestionSchema,
  AnalyzeResponseSchema,
  PlanProposalSchema,
