...
describe('POST /api/duplicates/detect', () => {
  ...
  it('runs detection without persisting by default', async () => {
    ...
    vi.mocked(detectDuplicates).mockResolvedValue({ groups: [{ id: 'g1' }], stats: { scanned: 10, candidates: 2, groups: 1 } } as any);
  ...
  it('handles empty results', async () => {
    vi.mocked(auth).mockResolvedValue({ user: { id: 'test-user' } } as any);
    vi.mocked(detectDuplicates).mockResolvedValue({ groups: [], stats: { scanned: 5, candidates: 0, groups: 0 } });
