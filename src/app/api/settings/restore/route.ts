...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Invalid backup file: not a valid tar.gz archive" },
      { status: 400 }
    );
  }
  ...
  const coachConversations = readJson<SerializedCoachConversation>(
    tmpDir,
          ...
          await tx.duplicateGroup.createMany({
            data: duplicateGroups.map((g: any) => ({
              ...
              keptActivityId: g.keptActivityId
                ? (trainingLogIdMap.get(g.keptActivityId) ?? g.keptActivityId)
                : null,
          ...
          await tx.trainingLog.createMany({
            data: activities.map((l: any) => ({
              ...
              duplicateGroupId: l.duplicateGroupId
                ? (duplicateGroupIdMap.get(l.duplicateGroupId) ?? null)
                : null,
        ...
        if (goals.length > 0) {
          await tx.raceGoal.createMany({
            data: goals.map((g: any) => ({
              ...
              status: g.status,
              notes: g.notes ?? null,
              goalStatement: g.goalStatement ?? null,
        ...
        if (weeklyAssessments.length > 0) {
          await tx.weeklyAssessment.createMany({
            data: weeklyAssessments.map((a: any) => ({
              ...
              fitnessScore: a.fitnessScore ?? null,
              fatigueScore: a.fatigueScore ?? null,
        ...
        if (garminSession) {
          await tx.garminSession.create({
            data: {
              ...
              lastHealthSyncAt: garminSession.lastHealthSyncAt
                ? new Date(garminSession.lastHealthSyncAt)
                : null,
