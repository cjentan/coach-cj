...
function AnalysisScheduleSection({
  t,
  common,
}: {
  ...
  return (
    <Card className="mb-6">
      ...
      <CardContent className="space-y-6">
          ...
          {trigger === "weekly" && (
            <div className="space-y-2">
              <Label className="flex items-center gap-1">
                <CalendarDays className="h-4 w-4" /> {t("dayOfWeek")}
              </Label>
        ...
        <div className="space-y-2">
          <Label className="flex items-center gap-1">
            <Clock className="h-4 w-4" /> {t("reviewTime")}
          </Label>
          <Input
            type="time"
            value={reviewTime}
        ...
        <div className="pt-4 border-t">
          ...
          <p className="text-sm mt-1">
            {lastAnalysisAt ? new Date(lastAnalysisAt).toLocaleString() : t("noAnalysis")}
          </p>
        </div>
        ...
        {saved && (
          <div className="text-sm text-green-600 flex items-center gap-1">
            <Check className="h-4 w-4" /> {settingsT("credentials.saved")}
          </div>
        )}
...
function AiProviderSection({
  ...
  async function runTest(testPrompt?: string) {
    ...
    try {
      ...
      setTestError(settingsT("credentials.testNetworkError"));
    } finally {
      setTesting(false);
    }
  }
      ...
      <CardContent className="space-y-4">
        {hasServerDefault && !hasStoredKey && !llmApiKey && !llmProvider && (
          <div className="p-3 rounded-md bg-blue-50 dark:bg-blue-950 text-blue-800 dark:text-blue-200 text-sm flex items-center gap-2">
            ...
            <span>
              {t("serverDefaultNotice", {
                provider: PROVIDER_LABELS[defaultProvider] || defaultProvider,
                model: defaultModel,
              })}
            </span>
          </div>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setLlmProvider(p);
              setLlmModel("");
              setLlmBaseUrl(PROVIDER_BASE_URLS[p] || "");
            }}
            ...
            <div className="rounded-md border">
              <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30">
                <div className="flex items-center gap-2 text-sm font-medium">
                  {testing ? (
                    <>
                      <Spinner className="h-4 w-4 animate-spin" /> {t("testing")}
                    </>
                  ) : result?.success ? (
                    <>
                ...
                {result && !testing && (
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    ...
                    {result.tokenEstimate && (
                      <span>
                        ~{result.tokenEstimate} {settingsT("credentials.tokens")}
                      </span>
                    )}
                  </div>
                )}
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">{settingsT("aiCoachDesc")}</p>
      </div>
