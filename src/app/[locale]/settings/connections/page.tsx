  ...
  const doSync = async () => {
    ...
    try {
      ...
      if (res.ok)
        setSyncResult({
          type: "success",
          text: t("garmin.syncComplete", {
            count: data.activitiesImported,
            healthDays: data.healthDaysSynced,
          }),
        });
        ...
        {status?.connected ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-muted-foreground">{t("garmin.activitiesSynced")}</span>
                <span className="ml-2 font-medium">{status.garminActivityCount}</span>
              </div>
              <div>
                <span className="text-muted-foreground">{t("garmin.connectedSince")}</span>
                <span className="ml-2 font-medium">
                  {status.connectedAt ? new Date(status.connectedAt).toLocaleDateString() : "—"}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">{t("garmin.lastActivitySync")}</span>
                <span className="ml-2 font-medium">
                  {status.lastSyncAt
              ...
              <div>
                <span className="text-muted-foreground">{t("garmin.lastHealthSync")}</span>
                <span className="ml-2 font-medium">
                  {status.lastHealthSyncAt
                    ? new Date(status.lastHealthSyncAt).toLocaleString()
                    : t("garmin.never")}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <Button onClick={doSync} disabled={syncing}>
                {syncing ? (
                  <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t("garmin.syncing")}
                  </>
                ) : (
                  <>
              ...
              <Button onClick={doDisconnect} variant="destructive" size="sm">
                <Unplug className="h-4 w-4 mr-2" /> {t("garmin.disconnect")}
              </Button>
              <Button onClick={doResetSync} variant="outline" size="sm">
                {t("garmin.resetSync")}
              </Button>
            </div>
            {syncResult && (
              <div
                className={`text-sm p-3 rounded-md ${syncResult.type === "success" ? "bg-green-50 dark:bg-green-950 text-green-800" : "bg-red-50 dark:bg-red-950 text-red-800"}`}
              >
                {syncResult.text}
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            {mfaRequired && (
              <div className="space-y-2">
                <Label>{t("garmin.mfaCodeLabel")}</Label>
                <Input
                  value={mfaCode}
            ...
            <Button
              onClick={doConnect}
              disabled={connecting || !email || !password || (mfaRequired && !mfaCode)}
            >
              {connecting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t("garmin.connecting")}
                </>
              ) : (
                <>
                  <Plug className="h-4 w-4 mr-2" />{" "}
                  {mfaRequired ? t("garmin.verifyMfa") : t("garmin.connect")}
                </>
              )}
            </Button>
            {connectError && <p className="text-sm text-destructive">{connectError}</p>}
            <p className="text-xs text-muted-foreground">
              {mfaRequired ? t("garmin.mfaLoginHelp") : t("garmin.passwordNote")}
            </p>
          </div>
        )}
      </CardContent>
...
function CorosSection({
  ...
  const doConnect = async () => {
    ...
    try {
      ...
      setConnectError(t("coros.networkError"));
    } finally {
      setConnecting(false);
    }
  };
  ...
  const doSync = async () => {
    ...
    try {
      ...
      if (res.ok)
        setSyncResult({
          type: "success",
          text: t("coros.syncComplete", { count: data.activitiesImported }),
        });
      else setSyncResult({ type: "error", text: t("coros.syncError", { error: data.error }) });
    } catch {
      setSyncResult({ type: "error", text: t("coros.syncNetworkError") });
    } finally {
      setSyncing(false);
    }
  };
      ...
      <CardContent>
        {status?.connected ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 font-medium">{status.corosActivityCount}</span>
              </div>
              <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <Button onClick={doSync} disabled={syncing}>
                {syncing ? (
                  <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t("coros.syncing")}
                  </>
                ) : (
                  <>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>{t("coros.emailLabel")}</Label>
              <Input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
            ...
            <div className="space-y-2">
              <Label>{t("coros.passwordLabel")}</Label>
              <Input
                type="password"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
            ...
            <Button onClick={doConnect} disabled={connecting || !email || !password}>
              {connecting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t("coros.connecting")}
                </>
              ) : (
                <>
                  <Plug className="h-4 w-4 mr-2" /> {t("coros.connect")}
                </>
              )}
            </Button>
            {connectError && <p className="text-sm text-destructive">{connectError}</p>}
...
function ApiKeysSection({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Key className="h-5 w-5" /> {t("apiKeys")}
        </CardTitle>
      ...
      <CardContent className="space-y-6">
        <div className="rounded-lg border p-4 space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="mt-1 font-mono bg-muted/50 p-2 rounded text-[11px]">
              Authorization: Bearer &lt;your_api_key&gt;
            </div>
          </div>
          <div>
            <span className="font-medium">{t("supportedFormats")}</span>
            <ul className="list-disc list-inside mt-1 space-y-0.5 text-xs text-muted-foreground">
              ...
              <li>
                <strong>FIT</strong> — use{" "}
                <code className="font-mono bg-muted px-1 rounded">-F file=@activity.fit</code>
              </li>
            </ul>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">{settingsT("connectionsDesc")}</p>
      </div>
